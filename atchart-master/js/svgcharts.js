...
class SVGCharts {
    constructor(colorTheme = 'good'){
        ...
        this.colors = this.colorTheme && this.colorTheme.length > 0?this.colorTheme[0].colors:this.colorThemes[0].colors;
    ...
    getColorsFromTheme(theme = 'blue'){
        let colorTheme = this.colorThemes.filter(_ => _.name === theme);
        let colors = colorTheme && colorTheme.length > 0 ? colorTheme[0].colors : this.colorThemes[0].colors;
        //colors = colors.reverse();        
        let res = '';
        for (let i=0; i<colors.length; i++){
            res = res + '<div style="background-color: '+colors[i]+';">'+i.toString()+'</div>';
        }
        return res;
    }
    ...
    getHorBarChart(data, title, drawLabels, barWidth, spacer){
        ...
        if (text_before){
            ...
            for (let i=0; i<data.length; i++){
                ...
                let itemValue = +item.value;
                y += +spacer;
                let percents = (itemValue*100/total).toFixed(2);
                let color = this.colors[(i+this.roller)%this.colors.length];
                let w = (maxX-maxText)*percents/100*scale;
                let x0 = maxText + (maxX-maxText-w)/2;
                res += '<rect x="0" y="'+y.toString()+'" width="'+w+'" height="'+barWidth.toString()+'" fill="'+color+'" />';
                if (drawLabels)
                    res += '<text x="' + (w + 3*this.textHeight).toString() + '" y="' + (y + barWidth/2+5).toString()+'" fill="#777" class="caption">'+itemLabel+', '+percents+'%</text>';
                y += +barWidth;
            }
            y += +spacer;
            maxX += 45;
    ...
    getPieBarChart(data, title, drawLabels, innerRadius){
        data.sort((a, b)=> b.value - a.value);
        let total = this._getTotalSum(data);
        let res = '';
    ...
    getFunBarChart(data, title, drawLabels, barWidth, spacer){
        data.sort((a, b)=> b.value - a.value);
        /*
        for (let i=0; i<data.length; i++){
